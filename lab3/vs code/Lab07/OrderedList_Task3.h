...
template<class T>
class OrderedList_Task3 :public OrderedList<T> {
public:
	/*Constructor*/
	OrderedList_Task3() :OrderedList<T>() {}
...
private:
	const int MAX_SIZE = 20; //needs to be changed to 50 and 10 while doing task 4
	T** data = new T * [MAX_SIZE]; //array of pointer elements
	int num_items = 0; //holds the number of items currently in the list
	int op_count = 0; //counts number of operation
};